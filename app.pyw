...
@app.route('/api/summary/daily/<string:start_date>/<string:end_date>', methods=['GET'])
def get_daily_time_summary(start_date, end_date):
    ...
    # Filter out weekends
    weekdays = [date for date in date_range if date.weekday() < 5]  # 0-4 are Monday-Friday
    ...
    daily_summary = db.session.query(
        ...
        func.round(
            func.sum(
                func.round(
                    (func.julianday(Task_Item.end_time) - func.julianday(Task_Item.start_time)) * 24 * 4
                )
            ) / 4, 2
        ).label('total_hours')
    ).filter(
        Task_Item.date >= start_date,
        Task_Item.date <= end_date,
        Task_Item.end_time.isnot(None)
    ).group_by(Task_Item.date).all()

    # Create a dictionary of existing data
    hours_by_date = {item.date: float(item.total_hours) for item in daily_summary}

    # Build result including all weekdays
    result = [
        {
            'date': date.strftime('%Y-%m-%d'),
            'total_hours': hours_by_date.get(date, 0.0)
        }
...
def summary_page():
    clients = Client.query.all()
    serialized_clients = [{'id': client.id, 'name': client.name} for client in clients]
    return render_template('time_summary.html', clients=serialized_clients)

@app.route('/api/summary/custom/<string:start_date>/<string:end_date>', methods=['GET'])
def get_custom_time_summary(start_date, end_date):
    ...
    summary = db.session.query(
        ...
        Task_Item.date < end_date,
        Task_Item.end_time.isnot(None)
    ).group_by(Task_Item.client_id, Client.name).all()

    result = [
        {
            'client_id': item.client_id,
            'client_name': item.client_name,
            'total_time': float(item.total_time)
        }
        for item in summary
    ]
...
class TaskItemModelView(ModelView):
    column_list = ['id', 'date', 'start_time', 'end_time', 'client', 'type', 'description', 'time_spent', 'adjust_entry']
    form_columns = ['date', 'start_time', 'end_time', 'client', 'type', 'description', 'time_spent', 'adjust_entry']
...
# Replace the existing Task_Item view with the new custom view
admin.add_view(TaskItemModelView(Task_Item, db.session))
...
if __name__ == '__main__':
    ...
    FlaskUI(app=app, server="flask", port=5001).run()